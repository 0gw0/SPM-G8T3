    ...
    return employees.map(employee => {
        ...
        if (employee.arrangements && employee.arrangements.length > 0) {
            employee.arrangements.forEach(arrangement => {
                ...
                if (arrangement.status === 'approved' && arrangement.start_date && arrangement.end_date) {
                    ...
                    if (arrangement.recurrence_pattern) {
                        ...
                        while (recurrenceStartDate < threeMonthsLater) {
                            if (arrangement.recurrence_pattern === 'monthly') {
                                recurrenceStartDate.setMonth(recurrenceStartDate.getMonth() + 1);
                                recurrenceEndDate.setMonth(recurrenceEndDate.getMonth() + 1);
                            } else {
                                recurrenceStartDate.setDate(recurrenceStartDate.getDate() + intervalDays);
                                recurrenceEndDate.setDate(recurrenceEndDate.getDate() + intervalDays);
                            }
