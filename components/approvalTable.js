...
function adjustTimeBasedOnType(data) {
  return data.map(employee => {
    const adjustedArrangements = employee.arrangements.map(arrangement => {
...
function removeNonPendingArrangements(data) {
  return data.map(employee => {
    const filteredArrangements = employee.arrangements.filter(arrangement => arrangement.status === "pending");
    return {
      ...employee,
...
const ApprovalTable = ({ arrangements, isLoading }) => {
  ...
  const handleApprove = (arrangement) => {
    console.log('Approved arrangement:', arrangement);
    setIsModalOpen(false);
    setRejectionReason("");
  };

  const handleReject = (arrangement) => {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {employeesWithArrangements.map((employee) => (
              employee.arrangements.map((arrangement) => (
                <tr key={`${employee.staff_id}-${arrangement.arrangement_id}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{arrangement.type}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{arrangement.status}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <button
                      onClick={() => {
                        setSelectedArrangement({ ...arrangement, employeeName: `${employee.staff_fname} ${employee.staff_lname}` });
                        setIsModalOpen(true);
                        setRejectionReason("");
                      }}
        ...
        {selectedArrangement && (
          <div className="pt-4">
            <h2 className="text-xl font-semibold mb-4">
              Arrangement Details - {selectedArrangement.employeeName}
            </h2>
            
            <div className="space-y-3 mb-6">
              <div className="grid grid-cols-3 gap-2">
                <div className="font-medium">Start Date:</div>
                <div className="col-span-2">{formatDate(selectedArrangement.start_date)}</div>
                
                <div className="font-medium">End Date:</div>
                <div className="col-span-2">{formatDate(selectedArrangement.end_date)}</div>
                
                <div className="font-medium">Pattern:</div>
                <div className="col-span-2">{selectedArrangement.recurrence_pattern}</div>
                
                <div className="font-medium">Type:</div>
                <div className="col-span-2">{selectedArrangement.type}</div>
                
                <div className="font-medium">Reason:</div>
                <div className="col-span-2">{selectedArrangement.reason}</div>
              </div>
            </div>
            ...
            <div className="flex justify-between gap-2 mt-6 pt-4 border-t">
              <button
                onClick={() => handleReject(selectedArrangement)}
                className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 focus:outline-none"
              >
                Reject
              </button>
              <button
                onClick={() => handleApprove(selectedArrangement)}
